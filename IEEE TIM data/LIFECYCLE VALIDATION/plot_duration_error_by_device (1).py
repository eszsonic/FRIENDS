...
not limits of agreement for individual puffs. All devices are included by
default. Devices with only one session are plotted as point estimates without
whiskers because their session-clustered confidence intervals cannot be
estimated.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "from the 'All Data' worksheet. All devices are included; devices "
            "with a single session are plotted without confidence intervals."
        )
    )
    parser.add_argument(
        "workbook",
    ...
    exclusion_group.add_argument(
        ...
        default=None,
        metavar="DEVICE",
        help="Device names to exclude (default: none).",
    )
    exclusion_group.add_argument(
        "--include-all-devices",
        action="store_true",
        help="Include all devices (now the default; retained for compatibility).",
    )
    parser.add_argument(
        "--bootstrap-mode",
...
def make_plot(
    ...
    for y_position, estimate in zip(y_positions, plotted):
        if math.isfinite(estimate.ci_low_s) and math.isfinite(estimate.ci_high_s):
            xerr = np.asarray(
                [
            ...
            # A session-clustered CI is not identifiable from one session.
            xerr = None
        ax.errorbar(
    ...
    legend_handles = [
        Line2D([0], [0], marker="o", color="#4B5563", markerfacecolor="#1F77B4", markersize=3.5, linewidth=0.8, label="Device mean (95% CI when n > 1)"),
    ...
    ax.annotate(
        ...
        "Whiskers: session-clustered CIs when n > 1. Dashed lines: descriptive limits.",
...
def main() -> None:
    ...
    if not estimates:
        raise ValueError("No devices remain to plot.")
